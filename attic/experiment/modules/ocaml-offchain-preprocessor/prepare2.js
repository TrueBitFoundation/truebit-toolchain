...
argv._.forEach(wasmFile => {
  let startPath = path.resolve(CWD, wasmFile)
  let endPath = path.resolve(tmp_dir, wasmFile).replace('/src/', '/')
  fs.copySync(startPath, endPath);
  wasm_file = endPath.replace(/.js$/, '.wasm');
  fs.copySync(startPath, path.resolve(tmp_dir, wasmFile));
});
